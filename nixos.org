...
So we force ~boot.loader.systemd-boot.enable~ to false for now.
#+begin_src nix
boot = {
  lanzaboote = {
    enable = true;
...
**** Languages
These tools are for non-serious usage and quick testing.
To seriously do development on nixos I have to write devshell.
#+begin_src nix
